...
void DGGraphics::RemoveAllBitmaps(void)
{
...
void FC DGGraphics::DrawBitmap(DGPoint& location, UINT bitmapID)
{
   ...
   assert(location.x + bitmap->GetWidth() < screenRes.x &&
      location.y + bitmap->GetHeight() < screenRes.y);
...
void DGGraphics::TestBitmap(char* fileName)
{
   HRESULT result;
   DGBitmap bitmap(1, 1, fileName);
   DGPoint bitmapSize = bitmap.GetDimensions();
   RECT destRect = {0, 0, bitmapSize.x, bitmapSize.y};
   result = lpDDSDrawingSurface->Blt(&destRect, bitmap.GetDDSurface(),
      NULL, DDBLT_WAIT, NULL);
   if(result != DD_OK)
      HandleDDrawError(EC_DDCLEARPRIMARY, result, __FILE__, __LINE__);
