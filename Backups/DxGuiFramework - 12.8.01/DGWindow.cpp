...
void FC DGWindow::SetPosition(const DGPoint& position)
{
   ...
   {
      iterator.GetData()->SendMessage(new DGMessage(GM_PARENTREPOSITION,
...
void FC DGWindow::DrawWindow()
{
   ...
   sprintf(outputString, "Clipped Area (%i, %i, %i, %i)\n", clippedArea.left,
      clippedArea.top, clippedArea.width, clippedArea.height);
   OutputDebugString(outputString);

   sprintf(outputString, "Abs Window Pos: (%i, %i)\n", absWindowPosition.x,
      absWindowPosition.y);
   OutputDebugString(outputString);

   sprintf(outputString, "windowOrigin: (%i, %i)\n", windowOrigin.x, windowOrigin.y);
   OutputDebugString(outputString);
