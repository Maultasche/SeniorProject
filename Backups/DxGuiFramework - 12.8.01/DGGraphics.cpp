...
DGGraphics::DGGraphics()
{
   ...
   textColor = DGColor(255, 255, 255);
   textBackgroundColor = DGColor(0, 0, 0);
      ...
      UINT bufferMode, bool persistence)
{
   ...
   DGColor::CreateLookupTables(pixelFormat, colorDepth);
...
Parameters:
   UINT bitmapID : the ID of the bitmap to be loaded
   UINT priority : the priority to be assigned to the loaded bitmap
   DGArea& area : the rectangular area of the bitmap that is to be
      loaded into memory.
Description:
   This function receives a bitmap ID, a priority, and a DGArea 
   object. The area specified by the DGArea object is loaded from
...
Parameters:
   ...
   DGArea& area : the rectangular area of the bitmap that is to be
      loaded into memory.
Description:
   This function receives a bitmap ID, a priority, a DGArea object
...
Parameters:
   UINT bitmapID : the ID of the bitmap to which the transparent color is
      to be set
   DGColor& transparentColor : the transparent color to be set
Description:
...
void FC DGGraphics::LockSurface()
{
...
Parameters:
   ...
   DGColor& color : the color that the pixel is to be set to
Description:
...
void FC DGGraphics::SetPixel(int x, int y, DGColor& color)
{
   ...
   switch(colorDepth)
   {
      ...
      case CD_24BIT:
      {
         ...
         memcpy((void*)videoBuffer[(x * 3) + (y * bufferPitch)],
            ((void*)((UCHAR*)&colorValue)[1]), 3);
      }
         break;
      case CD_32BIT:
...
Parameters:
   ...
   DGColor& color : the color that the line is to be set to
Description:
   ...
   for(int x = beginX; x <= endX; x++)
   {
      switch(colorDepth)
      {
         ...
         case CD_24BIT:
         {
            ...
            memcpy((void*)videoBuffer[bufferPosition],
               ((void*)((UCHAR*)&colorValue)[1]), 3);
         }
...
Parameters:
   DGPoint p1: the coordinate describing the beginning of the line
   DGPoint p2: the coordinate describing the end of the line
   DGColor& color : the color that the line is to be set to
Description:
...
Parameters:
   DGRectangle rect: a rectangle describing the upper-left and lower-
      right corners of the rectangle to be drawn
   DGColor& color : the color that the rectangle should be
Description:
...
void FC DGGraphics::DrawRectangle(DGRectangle& rect, DGColor& color)
{
   ...
   for(int x = beginX; x <= endX; x++)
   {
      switch(colorDepth)
      {
         ...
         {
            UINT colorValue = color.To24Bit();
            memcpy((void*)videoBuffer[bufferPositionFirstLine],
               ((void*)((UCHAR*)&colorValue)[1]), 3);
            memcpy((void*)videoBuffer[bufferPositionSecondLine],
               ((void*)((UCHAR*)&colorValue)[1]), 3);
         }
...
Parameters:
   ...
   DGColor& color : the color that the area should be filled with
Description:
...
void FC DGGraphics::FillArea(DGArea& area, DGColor& color)
{
   assert(surfaceLocked == false);
...
Parameters:
   DGColor& color : the color that the screen should be filled with
Description:
...
Parameters:
   DGPoint& location : the location at which to draw the bitmap, this
...
Parameters:
   DGArea& area: the area in which to draw the bitmap
   UINT bitmapID : the ID of the bitmap to be drawn
Description:
   This function draws the bitmap with the specified ID on the screen
...
void FC DGGraphics::DrawScaledBitmap(DGArea& area, UINT bitmapID)
{
   assert(surfaceLocked == false);
...
Parameters:
   DGPoint& location : the location at which to draw the bitmap, this
      is the coordinate where the upper-left corner of the bitmap will be.
   UINT bitmapID : the ID of the bitmap to be drawn
   DGColor& transparentColor : the color in the bitmap to be drawn 
      transparently
Description:
   This function draws the bitmap with the specified ID on the screen.
...
void FC DGGraphics::DrawTransparentScaledBitmap(DGArea& area, UINT bitmapID,
   DGColor& transparentColor)
{
   assert(surfaceLocked == false);
...
void DGGraphics::FlipToGDISurface()
{
...
Parameters:
   DGFont& font : the font to be used when drawing text
Description:
   This function sets the GDI font that the framework uses to draw text
   on the screen.
------------------------------------------------------------------------*/

void FC DGGraphics::SetGDIFont(DGFont& font)
{
   currentGDIFont = font;
}
...
void FC DGGraphics::SetTextTransparency(int mode)
{
...
Parameters:
   DGColor& color : the color to be used when drawing the background
      behind the text
Description:
   This function sets the color that the framework uses to draw the
...
void FC DGGraphics::SetTextBackgroundColor(DGColor& color)
{
...
Parameters:
   ...
   DGRectangle& rect : the bounding rectangle on the screen
...
void FC DGGraphics::DrawText(char* text, DGRectangle& rect, UINT flags)
{
   HDC hDC;
   HRESULT result;
   ...
   ::DrawText(hDC, text, -1, &rect.ToRECT(), flags | DT_EXPANDTABS |
      DT_NOPREFIX);
...
Parameters:
   DGRectangle& rect : the clipping area to be added
Description:
...
void FC DGGraphics::AddClippingArea(DGRectangle& rect)
{
...
//      rect.bottom >= 0 && rect.bottom < screenRes.y);
   ...
   //Store the clipping rect in a DGRectangle object so we can
   //easily use it later
   clippingRect = rect;
...
void FC DGGraphics::RemoveClippingArea()
{
...
void FC DGGraphics::ClearClippingAreas()
{
   RemoveClippingArea();
}
...
Parameters:
   DGArea& area : the area on the screen to represent as a surface
Description:
   This function receives an area describing an area of the screen
   for which a DGSurface object is to be created. Creating a surface will
   set a clipper, removing any previous clipper. As little as possible 
   should be done to the DGGraphics object until the surface is released. 
   The surface must be released after the caller is done drawing upon it.
------------------------------------------------------------------------*/

DGSurface* FC DGGraphics::GetSurface(DGArea& area)
{
   if(currentSurface != NULL)
      return NULL;
   
   currentSurface = new DGSurface(area);
   AddClippingArea(DGRectangle(area.left, area.top, area.Right(), 
      area.Bottom()));
...
Parameters:
   DGSurface* surface : the pointer to the surface to be released 
Description:
   This function releases the current surface. In case multiple surfaces
   are ever implemented, there is a DGSurface* parameter. At the moment,
   however, this pointer has no effect on the function.
------------------------------------------------------------------------*/

void FC DGGraphics::ReleaseSurface(DGSurface* surface)
{
   if(currentSurface == NULL)
      return;
