...
void FC DGWindow::DrawWindow()
{
   ...
   DGSurface* surface = dgGraphics->GetSurface(DGArea(windowPosition, 
      windowSize));

   //If the surface is NULL, someone else has not released their surface
   assert(surface != NULL);

   OnDrawWindow(surface);
...
Description:
   ...
   be destroyed by the framework.
------------------------------------------------------------------------*/

void FC DGWindow::OnDrawWindow(DGSurface* surface)
{
