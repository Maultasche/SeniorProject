...
Description:
   ...
   successfully.
Parameters: char* bitmapName - the name of a bitmap file from which a
               bitmap is to be loaded.
            int priority - the priority of the bitmap to be loaded
Preconditions: bitmapName points to a null-terminated character array
------------------------------------------------------------------------*/

void DGImage::LoadBitmapFile(char* bitmapName, int priority)
{
   ...
   //If there needs to be a transparent color
...
{
   if(parentNotify)
      DGGetGui()->PostMessage(new DGMessage(GM_IMAGE_LBUTTONDOWN,
...
{
   if(parentNotify)
      DGGetGui()->PostMessage(new DGMessage(GM_IMAGE_LBUTTONUP,
         GetParent()->GetWindowID(), 0, 0, GetWindowID()));
}
...
{
   if(parentNotify)
      DGGetGui()->PostMessage(new DGMessage(GM_IMAGE_LBUTTONDBLCLK,
         GetParent()->GetWindowID(), 0, 0, GetWindowID()));
}
...
{
   if(parentNotify)
      DGGetGui()->PostMessage(new DGMessage(GM_IMAGE_MBUTTONDOWN,
         GetParent()->GetWindowID(), 0, 0, GetWindowID()));
}
...
{
   if(parentNotify)
      DGGetGui()->PostMessage(new DGMessage(GM_IMAGE_MBUTTONUP,
         GetParent()->GetWindowID(), 0, 0, GetWindowID()));
}
...
{
   if(parentNotify)
      DGGetGui()->PostMessage(new DGMessage(GM_IMAGE_MBUTTONDBLCLK,
         GetParent()->GetWindowID(), 0, 0, GetWindowID()));
}
...
{
   if(parentNotify)
      DGGetGui()->PostMessage(new DGMessage(GM_IMAGE_RBUTTONDOWN,
         GetParent()->GetWindowID(), 0, 0, GetWindowID()));
}
...
{
   if(parentNotify)
      DGGetGui()->PostMessage(new DGMessage(GM_IMAGE_RBUTTONUP,
         GetParent()->GetWindowID(), 0, 0, GetWindowID()));
}
...
{
   if(parentNotify)
      DGGetGui()->PostMessage(new DGMessage(GM_IMAGE_RBUTTONDBLCLK,
         GetParent()->GetWindowID(), 0, 0, GetWindowID()));
}
