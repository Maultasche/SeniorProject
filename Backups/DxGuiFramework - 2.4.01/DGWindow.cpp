...
bool FC DGWindow::DestroyWindow(UINT winID)
{
   ...
   {
      bool found = false;
      DGWindow* childWindow = NULL;
      DGListIterator<DGWindow> iterator = windowList.Begin();

      while(!iterator.EndOfList() && !found)
      {
         if(iterator.GetID() == winID)
         {
            windowList.DeleteById(winID);
            found = true;
         }
         else
            found = iterator.GetData()->DestroyWindow(winID);
      ...
      while(!iterator.EndOfList() && !found)
      {
         if(iterator.GetID() == winID)
         {
            hiddenWindowList.DeleteById(winID);
            found = true;
         }
         else
            found = iterator.GetData()->DestroyWindow(winID);
