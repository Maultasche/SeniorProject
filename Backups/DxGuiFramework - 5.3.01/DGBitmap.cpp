...
void FC DGBitmap::SetTransparentColor(DGColor& color)
{
   ...
   if(lpDDSBitmap != NULL)
   {
      UINT colorDepth = dgGraphics->GetColorDepth();
      DDCOLORKEY colorKey;
      switch(colorDepth)
      {
         case CD_16BIT5:
      ...
      if(result != DD_OK)
         dgGraphics->HandleDDrawError(EC_DDCOLORKEY, result, __FILE__, __LINE__);
   }
}
...
void FC DGBitmap::LoadBitmap(UINT resourceID, DGRectangle& bitmapDimensions)
{
   //This function will be completed after resource files have been
   //implemented.
}
...
void FC DGBitmap::LoadBitmap(char* bitmapFileName, DGRectangle& bitmapDimensions)
{
   HRESULT result;
