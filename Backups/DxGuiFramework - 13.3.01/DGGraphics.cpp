...
Parameters:
   DGRectangle rect: the area in which to draw the bitmap, rect.left and
      rect.top describe the upper-left corner of the area and rect.right
      and rect.bottom describe the width and height of the area
   UINT bitmapID : the ID of the bitmap to be drawn
Description:
   This function draws the bitmap with the specified ID on the screen
   ...
   DGColor& transparentColor)
{
   ...
   switch(result)
   {
      case DDERR_SURFACELOST:
         RestoreAllSurfaces();
         DrawTransparentScaledBitmap(rect, bitmapID, transparentColor);
         break;
      case DD_OK:
         break;
      default:
...
void FC DGGraphics::DrawTransparentScaledBitmap(DGRectangle& rect, UINT bitmapID)
{
   ...
   switch(result)
   {
      case DDERR_SURFACELOST:
         RestoreAllSurfaces();
         DrawTransparentScaledBitmap(rect, bitmapID);
         break;
      case DD_OK:
         break;
      default:
...
//Clipping Functions
void FC DGGraphics::AddClippingArea(DGRectangle& area)
{
   assert(area.left >= 0 && area.left < screenRes.x &&
      area.right >= 0 && area.right < screenRes.x &&
   ...
   {
      tempVariable = area.left;
      area.left = area.right;
      area.right = tempVariable;
   }

   if(area.top > area.bottom)
   {
      tempVariable = area.top;
      area.top = area.bottom;
      area.bottom = tempVariable;
   }

   //Store the clipping area in a DGRectangle object so we can
   //easily use it later
   clippingRect = area;
   ...
   lpDDSDrawingSurface->SetClipper(lpDSClipper);
      clipping = true;
}
